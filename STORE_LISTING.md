# Chrome Web Store listing

The complete Chrome Web Store submission pack for AI Limits — store
configuration, artwork inventory, listing copy, permission justifications,
data-disclosure answers, reviewer prerequisites, and the twenty-provider
test flow — lives in:

- [English listing pack](store-assets/chrome-web-store/listing-en.md)
- [简体中文 listing pack](store-assets/chrome-web-store/listing-zh_CN.md)

Chrome Web Store review is conducted against the English pack.
