...
describe("built Chrome artifact contract", () => {
  it("accepts exactly five optional provider origins and the store description", () => {
    expect(EXPECTED_DESCRIPTION.length).toBeLessThanOrEqual(132);
    expect(EXPECTED_OPTIONAL_ORIGINS).toHaveLength(5);
    expect(validateBuildManifest(validManifest(), "0.2.1")).toEqual([]);
  });
  ...
  it("rejects missing API access, broad ElevenLabs webpage access, and tabs", () => {
    ...
    expect(validateBuildManifest(missingApi, "0.2.1")).toContain(
      "Expected the five exact optional provider origins.",
    );

    const broadPage = validManifest();
    broadPage.optional_host_permissions.push("https://elevenlabs.io/*");
    expect(validateBuildManifest(broadPage, "0.2.1")).toContain(
      "Expected the five exact optional provider origins.",
    );

    const tabs = validManifest();
    tabs.permissions.push("tabs");
    expect(validateBuildManifest(tabs, "0.2.1")).toContain(
